			       ...
			       int cpu, int max_freq)
{
	...
	if (!ret)
		pr_info("[THERM]: Limiting core%d max frequency to %d\n",
			cpu, max_freq);
...
static void check_temp(struct work_struct *work)
{
	...
	if (ret) {
		pr_err("[THERM]: Unable to read TSENS sensor %d\n",
	...
	for_each_possible_cpu(cpu) {
		...
		if (!cpu_policy) {
			pr_debug("[THERM]: NULL policy on cpu %d\n", cpu);
			continue;
		}
		...
		if ((temp >= msm_thermal_tuners_ins.allowed_low_high) &&
			...
			thermal_throttled = 1;
			pr_warn("[THERM]: Thermal Throttled (low)! temp: %lu\n", temp);
		//low clr point
			...
			if (cpu_policy->max < cpu_policy->cpuinfo.max_freq) {
				if (pre_throttled_max != 0)
					...
					pr_warn("[THERM]: ERROR! pre_throttled_max=0, falling back to %u\n", max_freq);
				}
				update_policy = 1;
				/* wait until 2nd core is unthrottled */
				if (cpu == 1)
					thermal_throttled = 0;
				pr_warn("[THERM]: Low Thermal Throttling Ended! temp: %lu\n", temp);
			}
		//mid trip point
			...
			max_freq = msm_thermal_tuners_ins.allowed_low_freq;
			thermal_throttled = 2;
			pr_warn("[THERM]: Thermal Throttled (mid)! temp: %lu\n", temp);
		//mid clr point
			...
			if (cpu_policy->max < cpu_policy->cpuinfo.max_freq) {
				...
				if (cpu == 1)
					thermal_throttled = 1;
				pr_warn("[THERM]: Mid Thermal Throttling Ended! temp: %lu\n", temp);
			}
		//max trip point
			...
			thermal_throttled = 3;
			pr_warn("[THERM]: Thermal Throttled (max)! temp: %lu\n", temp);
		//max clr point
			...
			if (cpu_policy->max < cpu_policy->cpuinfo.max_freq) {
				...
				if (cpu == 1)
					thermal_throttled = 2;
				pr_warn("[THERM]: Max Thermal Throttling Ended! temp: %lu\n", temp);
			}
		}
...
static int set_enabled(const char *val, const struct kernel_param *kp)
{
	...
	if (!enabled)
		...
		pr_info("[THERM]: no action for enabled = %d\n", enabled);

	pr_info("[THERM]: enabled = %d\n", enabled);
